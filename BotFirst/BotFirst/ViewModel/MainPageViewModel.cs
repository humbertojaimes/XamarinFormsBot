...
namespace BotFirst.ViewModel
{
	class MainPageViewModel : INotifyPropertyChanged
	{
		...
		public async void SendMessage()
		{
			...
			var result = await PostAsync(botUriChat, content);
			postResult = JsonConvert.DeserializeObject<PostResult>(result);
			...
			if (postResult != null)
			{
				...
				if (firstMessage)
				{
					...
					firstMessage = false;
					lastMessage = int.Parse(getResult.watermark);
				}
				...
				else
				{
					...
					receivedMessages = int.Parse(getResult.watermark) - lastMessage;
				}
				...
				foreach (var activity in getResult.activities)
				{
					if (!activity.from.id.ToLowerInvariant().Equals("user1"))
					{
						if (!string.IsNullOrEmpty(activity.text))
							myChat.Add("Bot: " + activity.text);
						else if (activity.attachments != null)
						{
							StringBuilder message = new StringBuilder();
							message.AppendLine(activity.attachments[0].content.text);
							foreach (var button in activity.attachments[0].content.buttons)
							{
								message.AppendLine(string.Format($"{button.value} - {button.title}"));
							}
							myChat.Add("Bot: " + message);
						}
					}
				}
